            ...
            {state.routes.map((route, index) => {
                ...
                return (
                    <Pressable key={route.key} className={`flex flex-row p-2 flex-1 justify-center items-center gap-2`}>
                        <Pressable
                            className={`p-2 flex flex-row items-center justify-center w-full rounded-xl gap-1 ${isFocused && 'bg-dark'}`}
                            onPress={() => navigation.navigate(route.name)}
                        >
                            <Icon name={isFocused ? `${iconName}-outline` : iconName} size={24} color={isFocused ? colorPallet.primary : colorPallet.dark} />
